...
void ymz280b_device::generate_sample(stream_sample_t **outputs, int samples)
{
	...
	else
	{
		switch (m_current_register)
		{
			...
			case 0x87:      /* RAM write */
				if (m_ext_mem_enable)
				{
					write_byte(m_ext_mem_address, data);
...
ymz280b_device::ymz280b_device(uint32_t clock) : m_master_clock((double)clock / 384.0)
	, m_current_register(0)
	, m_status_register(0)
	, m_irq_state(0)
	, m_irq_mask(0)
...
void ymz280b_device::register_write(uint32_t address, uint32_t data)
{
	switch (address & 0x01) {
	case 0:
		m_current_register = (uint8_t)(data & 0xff);
		break;
	case 1:
		write_to_register((int)data);
		break;
	}
}

uint32_t ymz280b_device::register_read(uint32_t address)
{
	return 0xff;
}